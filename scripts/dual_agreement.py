...
from .score import (CORRECT_A, CORRECT_B, INCORRECT_A, MISSING_A, MISSING_B,
                    PARTIAL_A, SPURIOUS_A, SPURIOUS_B)
...
def concepts_agreement(data, all=True):
    assert all or not data[INCORRECT_A], 'For a single concept class, no incorrect matches are allowed'

    c_score = len(data[CORRECT_A])
    p_score = sum(partial_score(a,b) for a,b in data[PARTIAL_A].items())
    n = sum(len(data[x]) for x in [CORRECT_A, PARTIAL_A, MISSING_A, SPURIOUS_A, INCORRECT_A])
...
def compute_metrics(data1, data2):
    ...
    return {
        ...
        'relations_agreement': (c_score11 + c_score22) / (n11 + n22) if n11 + n22 else 1.,
        'agreement': (c_score111 + p_score111 + c_score222 + p_score222) / (n111 + n222)
    }
